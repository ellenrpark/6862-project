...
plt.ylabel('# Data points')
plt.xlabel('Month')
plt.title('GO-SHIP Surface Data Histogram (n = '+str(GOSHIPData.shape[0])+')')
plt.savefig(FigDir+'GOSHIPData_histogram_surfdata.jpg')
plt.clf(); plt.close()
...
for i in np.arange(len(var_list)):
    ...
    plt.colorbar(A)
    plt.title('GO-SHIP Surface Values Map: '+var_list_full[i]+' ('+units[i]+')')
    plt.savefig(FigDir+'GOSHIPData_map_surface_'+var_list_full[i]+'.jpg')
    plt.clf(); plt.close()
    ...
    plt.title('GO-SHIP Surface Time Series: '+var_list_full[i])
    plt.ylabel(var_list_full[i]+' ('+units[i]+')')
    plt.xticks(rotation=45)
    plt.savefig(FigDir+'GOSHIPData_timeseries_surface_'+var_list_full[i]+'.jpg')
    plt.clf(); plt.close()
